...
1. 已安裝 AltspaceVR 和 OBS。 以2D 模式啟動 AltspaceVR、啟動 OBS、將 OBS 設定為記錄 AltspaceVR 並記錄離開！
...
| **串流** | 保留預設值 |
| 輸出模式 | 切換至 Advanced |
| [資料流程] 索引標籤 | 音訊曲目1 <br> 編碼器： x264 <br> 重新調整輸出：未核取 <br> 速率控制： CBR <br> 位元速率： 6000 (6000，適用于 60 fps) 30 fps 或9000 <br> 主要畫面格間隔 = 2 <br> CPU 使用量預設值 = veryfast |
| 錄製索引標籤 | 類型：標準 <br> 錄製路徑： D：/Video (流覽至您想要儲存影片檔案的位置)  <br> 錄製格式：有可能會在錄製時出現某些損毀的 (。如果您損毀影片仍可搭配使用 flv，請在這裡嘗試使用 flv)  <br> 音訊曲目1 <br> 編碼器：使用串流編碼器 |
...
3. 查看桌面音訊和 Mic/Aux 層級，因為這樣做可查看是否正常運作。
...
4. 在 [來源] 底下，選取 [ **+** 遊戲捕捉]、[建立新的]、[編輯文字] 至 [AltspaceVR Capture]、勾選 [讓來源可見、正常]
5. 按兩下 AltspaceVR Capture
6. 模式：捕獲特定視窗
7. 視窗： [AltspaceVR.exe]： AltspaceVR
8. 視窗符合優先順序：符合標題，否則尋找相同可執行檔的視窗
9. 向下滾動以捕捉資料指標： untick 確定
...
<br>好的，現在請務必按一下 [套用]，再按一下 [確定]，然後關閉並重新開啟 OBS。 這會儲存您所有的 OBS 設定。 美觀： ) 
...
您可以快速取得即時串流 (**stream**) 或設定未來的即時串流事件， (**管理**) 。 我可能會建議您以「管理」方式設定它。

1. 開啟您的瀏覽器並登入 [https://www.youtube.com/](https://www.youtube.com/) ，然後前往 [https://www.youtube.com/my_live_events](https://www.youtube.com/my_live_events)
...
* 套用，然後確定<br>
* 選取 [在 OBS 中啟動串流]<br>
* 切換至 YouTube，您將會看到您現在已上線至 YouTube！<br>
* 若要查看您實際的 YouTube live stream 影片頁面，您必須選取右上方的共用圖示。<br>
* 複製「影片連結」並貼到新的瀏覽器索引標籤中，您會看到您的 YouTube 即時資料流頁面。<br>
* 此 URL 是您的即時串流連結，可與您的所有社交頻道共用： ) <br>
* 若要停止即時資料流，請選取 OBS 上的 [停止串流]，這會結束 YouTube 上的即時串流。<br>
* 然後在 YouTube 上停止串流<br>
